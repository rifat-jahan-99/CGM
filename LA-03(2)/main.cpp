...
int main()
{
    ...
    // --------------------
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Rifat", NULL, NULL);
    if (window == NULL)
    {
    ...
    unsigned int fragmentShaderCyan = glCreateShader(GL_FRAGMENT_SHADER); // the first fragment shader that outputs the color Blue
    unsigned int fragmentShaderMagenta = glCreateShader(GL_FRAGMENT_SHADER); // the second fragment shader that outputs the color Green
    unsigned int shaderProgramCyan = glCreateProgram();
    unsigned int shaderProgramMagenta = glCreateProgram(); // the second shader program
